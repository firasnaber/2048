    ...
    public static boolean atLeastOneMoveExists(Board b) {
        ...
        for (int i = 0; i < size; i++) {
            for (int j = 0; j < size; j++) {
                int curVal = b.tile(j, i).value();
                if (j < size - 1 && curVal == b.tile(j + 1, i).value()) return true;
                if (i < size - 1 && curVal == b.tile(j, i + 1).value()) return true;
            }
        }
        return false;
    }
